...
|-----------|--------------|----------|
| `PostToolUse` | After any tool is used | Quality checks, validation, logging |
| `PreUserPromptSubmit` | Before user's message is sent | Modify prompts, add context |
| `PostUserPromptSubmit` | After user's message is sent | Logging, analytics |
